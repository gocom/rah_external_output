...
p. While these snippets can act as pages, created snippets are independent from Textpattern's normal content model. Snippets won't required or overpopulate "page templates":http://textpattern.net/wiki/index.php?title=Pages or "sections":http://textpattern.net/wiki/index.php?title=Sections.

p. This makes the plugin ideal for generating various publicly accessible snippets, including XML feeds or JavaScript Object Notations that can be then used to feed site's content to JavaScript applications or to transmit content across domains.

p. Rah_external_output uses Textpattern's "form partials":http://textpattern.net/wiki/index.php?title=Forms to power its snippets and public accessible pages. A form partial that's name is prefixed with @rah_eo_@ can be accessed directly with a URL, creating its own page. The term _external_ directly refers to separateness and independency from sections and page templates while still serving client-side content.
...
p. Rah_external_output treats misc type "form partials":http://textpattern.net/wiki/index.php?title=Forms that are prefixed with @rah_eo_@ prefix as a "external" content-type snippets. These special prefixed forms can be accessed using a public callback URL. This URL would follow a pattern:
...
p. Creating a new rah_external_output's custom form would follow same steps as "any other form":http://textpattern.net/wiki/index.php?title=Forms#Creating_a_New_Form. Go to Forms panel, located under "Presentation":http://textpattern.net/wiki/index.php?title=Presentation section, and click the _Create new form_ button at the top of the column on the right. This will open a new empty editor page.

p. You can type in the large code field any Textpattern markup you wish like with a normal form. This is what your snippet will output when accessed using a callback URL. For now, type in the field this:

bc. ; Content-type: text/plain
<<txp:site_name /> says> Hello World!

p. You can then give the snippet a name type in it in the _Name_ field. This name will be also used for the snippet's URL. To create a rah_external_output's snippet, the name should be prefixed with @rah_eo_@. For now, give the form a name @rah_eo_hello_world@.

p. Last but not least is a _Type_. The most appropriate type a snippet is a _misc_, but it can be anything if you so wish. Form types are, for the most part, visual, used for organizing things on the admin-side, for _Write_ panel's _Output form_ field and for _Tag Builder_.

p. Hit _Save_. The snippet is now ready for being accessed. After you have saved the editor, you will that there is _View_ link next to the _Name_ field. By clicking that link you will be taken to the snippet's publicly accessible location which you can then use in your content served to clients, like JavaScript, or to fetch content cross domains.

p. For the snippet, @rah_eo_hello_world@, you just created, this URL would be:

bc. http://example.com/?rah_external_output=hello_world

p. Where @example.com@ is your site's URL and @hello_world@ the name of the snippet. When accessing the snippet's URL, you should see following:

bc. <MySiteName says> Hello World!

p. Where @MySiteName@ is the name of the site. As you notice the, the page is served as a plain text opposed to HTML. This is due to a HTTP header the snippet's code included:
...
p. In addition to a form partial's normal features, rah_external_output's special forms support sending custom HTTP headers, including a content-type header.
...
p. If a form is named as a @rah_eo_jsontable.json@, the content would be served as a JSON presentation when @http://example.com/?rah_external_output=jsontable.json@ is being accessed.
...
p. The plugin comes with couple things targeted to developers. One of these is a dedicated callback event which is fired when a snippet is viewed. The callback in question is named @rah_external_output.snippet_end@ and is executed at the very end of a snippet.
