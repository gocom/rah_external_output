...
p. Rah_external_output uses Textpattern's "form partials":http://textpattern.net/wiki/index.php?title=Forms to power its snippets. A form partial that's name is prefixed with @rah_eo_@ can be accessed directly with a URL, creating its own, independent, page. The term _external_ directly refers to separateness and independency from sections and page templates while still serving client-side content.
...
* Dropped code path used as plugin cache fallback. Now relays on existence of plugin-lifecycle callbacks.
* Dropped migration cleaner deployed in v0.6. Left over table @rah_external_output_mime@ can be dropped manually.
