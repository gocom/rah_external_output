...
p. All lines at the beginning of the form starting with semicolon (@;@) are sent as HTTP headers. The lines can should be placed at the beginning of a form, starting from the first line. HTTP header lines and can not be preceded by any white-space or linebreaks. HTTP header lines follow the normal syntax you would expect from a HTTP header field definitions. A line consist of header property, the field, followed by a colon (@:@), followed by a value.

bc. ; Property: Value

p. A form supports multiple number of header lines. Each subsequent line starting with a semicolon is sent as a HTTP header.
...
p. Using these HTTP header lines, visitor can be redirected, or document's content-type, encoding, caching or timestamp attributes can be changed. A form that starts with following header line would be sent as a plain-text.
...
p. Note that for security reasons, HTTP header lines do not allow Textpattern tags. Headers are collected from the form before any Textpattern tags are parsed. This to prevent any type of header injections that could occur otherwise.
